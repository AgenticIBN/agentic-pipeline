...
class IntentParse(BaseModel):
    ...
    target_kpis: list = Field(default_factory=list, description="List of KPIs to optimize (RX_POWER, SINR, COVERAGE, THROUGHPUT_5P, etc.)")
...
INSTRUCTIONS = [
    ...
    "- 'turn off TX2', 'deactivate TX2', 'shut down TX2', 'TX2 maintenance' → Add 'TX2 base station' to target_area AND add 'COVERAGE' to target_kpis (maintaining coverage is the goal when turning off TX)",
    "- 'turn on TX1', 'activate TX1', 'enable TX1' → Add 'TX1 base station' to target_area AND add 'COVERAGE' to target_kpis",
    ...
    "- When turning OFF transmitters, add kpi_threshold with op='TARGET' and kpi='COVERAGE' (no value needed - system will maximize)",
    "",
    "## KPI Mapping Rules:",
    ...
    "- Maintenance/Turn off/Turn on/Activate/Deactivate → COVERAGE (the goal when changing TX state)",
    "",
    "## Operator Mapping:",
    ...
    "- 'maximize', 'optimize', 'improve', 'maintain' without specific value → TARGET (leave value field empty/omit it entirely)",
    "- 'target', 'aim for' with specific number → TARGET with numeric value",
    "",
    "## Priority Inference:",
    ...
    "- For optimization requests without specific thresholds: use TARGET operator and OMIT the value field entirely (don't use 'null' string)",
    "- NEVER use string values like 'max', 'min', 'optimize', 'null', 'high' in the value field",
    "- Value field must be: numeric, or completely omitted/None (not the string 'null')",
]
